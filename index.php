...
<head>
    ...
    if (isset($_GET['steamid'])) {
        $data = json_decode(file_get_contents("https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=FC9B9C932ABBEA822932A809CF069B42&steamids=" . $steamid . ""));
        $main = $data->response->players[0];
        if (is_null($main->personaname)) {
            //is null
        } else {
            $plname = $main->personaname;
        }
        if (is_null($main->avatarfull)) {
            //is null
        } else {
